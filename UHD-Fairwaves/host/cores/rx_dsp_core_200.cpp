...
public:
    ...
    void clear(void) {
        _iface->poke32(REG_RX_CTRL_NCHANNELS, 0);
    ...
    void issue_stream_command(const stream_cmd_t& stream_cmd) {
        if (not _initialized)
        {
            UHD_LOGGER_ERROR("UMTRX") << "issue_stream_command called on uninitialized rx_dsp_core";
            return;
        }
        ...
        _iface->poke32(REG_RX_CTRL_TIME_LO, boost::uint32_t(ticks >> 0));
    }
    ...
    void set_link_rate(const double rate) {
        _link_rate = rate / sizeof(boost::uint16_t);
    }

    uhd::meta_range_t get_host_rates(void) {
        meta_range_t range;
        for (int rate = 512; rate > 256; rate -= 4) {
            range.push_back(uhd::range_t(_tick_rate / rate));
        }
    ...
    double set_host_rate(const double rate) {
        ...
        {
            UHD_LOGGER_WARNING("UMTRX")
                << "The requested decimation is odd; the user should expect CIC rolloff."
                << " Select an even decimation to ensure that a halfband filter is enabled."
                << " decimation = dsp_rate/samp_rate -> " << decim_rate
                << " = (" << (_tick_rate / 1e6) << " MHz)/(" << (rate / 1e6) << " MHz)";
        }
